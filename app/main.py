    ...
    if _img_off:
        ...
        if settings.face_swap_enabled:
            ...
            face_swap_service = FaceSwapService(
                ...
                hf_pixel_boost=getattr(settings, 'hf_face_swap_pixel_boost', None) or '1024x1024',
                post_upscale=float(getattr(settings, 'face_swap_post_upscale', 2.0) or 1.5),
                min_target_side=int(getattr(settings, 'face_swap_min_target_side', 1024) or 768),
                max_output_side=int(getattr(settings, 'face_swap_max_output_side', 2048) or 2048),
                jpeg_quality=int(getattr(settings, 'face_swap_jpeg_quality', 97) or 97),
                face_sharpen=float(getattr(settings, 'face_swap_face_sharpen', 1.8) or 1.8),
