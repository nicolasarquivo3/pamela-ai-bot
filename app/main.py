    ...
    if _img_off:
        ...
        if settings.face_swap_enabled:
            ...
            face_swap_service = FaceSwapService(
                ...
                hf_enhance_enabled=bool(getattr(settings, "hf_face_enhance_enabled", True)),
                hf_enhance_space=getattr(settings, "hf_face_enhance_space", None) or "sczhou/CodeFormer",
                hf_enhance_api_name=getattr(settings, "hf_face_enhance_api_name", None) or "/predict",
    ...
    if face_swap_service is None:
        try:
            ...
            face_swap_service = FaceSwapService(
                ...
                hf_restore_strength=float(getattr(settings, "hf_face_restore_strength", 0.5) or 0.5),
                hf_enhance_enabled=bool(getattr(settings, "hf_face_enhance_enabled", True)),
                hf_enhance_space=getattr(settings, "hf_face_enhance_space", None) or "sczhou/CodeFormer",
                hf_enhance_api_name=getattr(settings, "hf_face_enhance_api_name", None) or "/predict",
